...
QuadrotorLQRControl::QuadrotorLQRControl()
{
    ...
    //_K = readMatrixK("/home/raffaele/PX4/Firmware/src/modules/mc_att_control/lqr_files\new_controller.txt");
    _K = readMatrixK("C:/PX4/home/Firmware/src/modules/mc_att_control/lqr_files/new_controller_LQR.txt");
    //_K = readMatrixK("C:/PX4/home/Firmware/src/modules/mc_att_control/lqr_files/matlab_cont.txt");

    //_P = readMatrixP("/home/raffaele/PX4/Firmware/src/modules/mc_att_control/lqr_files\new_pe.txt");
    //_P1 = readMatrixP("C:/PX4/home/Firmware/src/modules/mc_att_control/lqr_files/new_pe.txt");
    ...
    outfile1.open("C:/PX4/home/Firmware/src/modules/mc_att_control/output_files/control_input.txt", std::ios::out);
    ...
    outfile3.open("C:/PX4/home/Firmware/src/modules/mc_att_control/output_files/state.txt", std::ios::out);
    outfile3.close();


    ofstream outfile5;
    //outfile5.open("/home/raffaele/PX4/Firmware/src/modules/mc_att_control/output_files/lyapunov.txt", std::ios::out);
    outfile5.open("C:/PX4/home/Firmware/src/modules/mc_att_control/output_files/lyapunov.txt", std::ios::out);
    outfile5.close();

    ofstream outfile4;
    //outfile4.open("/home/raffaele/PX4/Firmware/src/modules/mc_att_control/output_files/ekf.txt", std::ios::out);
    outfile4.open("C:/PX4/home/Firmware/src/modules/mc_att_control/output_files/ekf.txt", std::ios::out);
    outfile4.close();
...
Matrix<float, 4, 1> QuadrotorLQRControl::LQRcontrol()
{
    ...
    _eq_point(5, 0) = -0.1 * time_ref; //z
    _eq_point(4, 0) = -0.1; //zdot
    _eq_point(1, 0) = -1 * cos(0.5 * time_ref); //x
    _eq_point(0, 0) = 0.5 * sin(time_ref); //xdot
    _eq_point(3, 0) = -1 * sin(0.5 * time_ref); //y
    _eq_point(2, 0) = -0.5 * cos(time_ref); //ydot
    ...
    // cout << _eq_point(0,0) << _eq_point(1,0) << _eq_point(2,0) << _eq_point(3,0) << _eq_point(4,0) << _eq_point(5,0) << endl;

    //cout << _K(2,1) << ", " << _K(1,4) << endl;
   ...
   // !! IMPORTANT scale the control inputs.......
    ...
    const float mass = 0.8; // quadcopter mass (kg)
    const float gravity = 9.8; // gravity (m/s^2)
    const float ff_term = mass * gravity;


    u_control_norm(1, 0) = fmin(fmax((u_control(1, 0)) / (0.1080f * 4.0f), -1.0f), 1.0f);
  //  u_control_norm(2, 0) = fmin(fmax((u_control(2, 0)) / (0.1080f * 4.0f), -0.05f), 0.05f);
   ...
   // u_control_norm(3, 0) = fmin(fmax((u_control(3, 0)) / (0.1f * 1.0f), -1.0f), 1.0f);
    u_control_norm(3, 0) = fmin(fmax((u_control(3, 0)) / (0.1080f * 4.0f), -1.0f), 1.0f);
   ...
   // u_control(3, 0) = u_control_norm(3, 0) * 0.05f;
    ...
    writeLyapunovOnFile("C:/PX4/home/Firmware/src/modules/mc_att_control/output_files/lyapunov.txt", _lyap_fun(0, 0), now);
    writeStateOnFile("C:/PX4/home/Firmware/src/modules/mc_att_control/output_files/ekf.txt", _current_state_ekf, now);
...
Matrix <float, 12, 12>  QuadrotorLQRControl::readMatrixP(const char* filename)
{
    ...
    static Matrix <float, 12, 12> result1;
    static int rows = 12;
    static int cols = 12;
    ifstream infile;
    infile.open(filename);
